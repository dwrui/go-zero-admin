...
func GetQuickList(ctx context.Context, svcCtx *svc.ServiceContext, businessId uint64) (list []*common.GetQuickRow, err error) {
	...
	var businessAuthQuickop []*BusinessAuthQuickop
	resp := svcCtx.DB.Model("business_home_quickop").Where(ga.Map{"business_id": businessId}).Select(ctx, &businessAuthQuickop)
	if resp.GetError() != nil {
		return nil, resp.GetError()
	}
	list = ConvertToGetQuickRow(businessAuthQuickop)
	return list, nil
}

// Convert
func ConvertToGetQuickRow(src []*BusinessAuthQuickop) []*common.GetQuickRow {
	// 预分配容量（性能优化关键）
...
func SaveQuick(ctx context.Context, svcCtx *svc.ServiceContext, req *common.SaveQuickRequest) (any, error) {
	resp := svcCtx.DB.Model("business_home_quickop").Save(ctx, req)
	if resp.GetError() != nil {
		return nil, resp.GetError()
	}
