...
// 获取权限菜单
func GetMenuArray(ctx context.Context, svg *svc.ServiceContext, pdata []BusinessAuthRule, parent_id int64, roles []interface{}) []map[string]interface{} {
	...
	for _, v := range pdata {
		if ga.Int64(v.Pid) == parent_id {
			...
			//11.按钮权限
			if len(roles) == 0 { //超级权限
				permission := svg.DB.Model("business_auth_rule").Where("status", 0).Where("type", 2).Where("pid", v.Id).WhereNotNull("permission").Column(ctx, "permission")
				...
				permission := svg.DB.Model("business_auth_rule").Where("status", 0).Where("type", 2).Where("pid", v.Id).WhereIn("id", roles).WhereNotNull("permission").Column(ctx, "permission")
				if permission.IsNotEmpty() && len(permission.GetData().([]string)) > 0 {
					meta["btnroles"] = permission.GetData().([]string)
				} else {
					hasepermission := svg.DB.Model("business_auth_rule").Where("status", 0).Where("type", 2).Where("pid", v.Id).WhereNotNull("permission").Column(ctx, "permission")
