...
func GetRuleAll(ctx context.Context, svcCtx *svc.ServiceContext, field string, order string) ([]*BusinessAuthRuleModel, error) {
	var rule []*BusinessAuthRuleModel
	resp := svcCtx.DB.Model("business_auth_rule").Fields(field).OrderBy(order).Select(ctx, &rule)
	if resp.GetError() != nil {
		return nil, resp.GetError()
	}
	return rule, nil
}
func GetRuleOne(ctx context.Context, svcCtx *svc.ServiceContext, field string, id uint64) (*BusinessAuthRuleModel, error) {
	var rule *BusinessAuthRuleModel
	resp := svcCtx.DB.Model("business_auth_rule").Where("id", id).Fields(field).Find(ctx, &rule)
	if resp.GetError() != nil {
		return nil, resp.GetError()
	}
	return rule, nil
...
func SaveRule(ctx context.Context, svcCtx *svc.ServiceContext, data *system.SaveRuleRequest) (uint64, error) {
	var businessAuthRule BusinessAuthRuleModel
	resp := svcCtx.DB.Model("business_auth_rule").Data(data).Save(ctx, &businessAuthRule)
	if resp.GetError() != nil {
...
func UpStatus(ctx context.Context, svcCtx *svc.ServiceContext, data *system.UpStatusRuleRequest) error {
	resp := svcCtx.DB.Model("business_auth_rule").Where("id", data.Id).Update(ctx, ga.Map{"status": data.Status})
	if resp.GetError() != nil {
...
func Del(ctx context.Context, svcCtx *svc.ServiceContext, data *system.DelRuleRequest) error {
	resp := svcCtx.DB.Model("business_auth_rule").Where("id", data.Id).Delete(ctx)
	if resp.GetError() != nil {
		return resp.GetError()
	}
	return nil
}
func GetParentAll(ctx context.Context, svcCtx *svc.ServiceContext, whereIn ga.Slice, where ga.Map, field string, order string) ([]*BusinessAuthRuleModel, error) {
	var rule []*BusinessAuthRuleModel
	resp := svcCtx.DB.Model("business_auth_rule").WhereIn("type", whereIn).Where(where).Fields(field).OrderBy(order).Select(ctx, &rule)
	if resp.GetError() != nil {
		return nil, resp.GetError()
	}
	return rule, nil
...
func GetRoutesAll(ctx context.Context, svcCtx *svc.ServiceContext) ([]*BusinessAuthRuleModel, error) {
	var rule []*BusinessAuthRuleModel
	resp := svcCtx.DB.Model("business_auth_rule").Where("path != ?", "").Column(ctx, "path")
	if resp.GetError() != nil {
		return nil, resp.GetError()
	}
	return rule, nil
