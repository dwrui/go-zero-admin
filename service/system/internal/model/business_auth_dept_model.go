...
type BusinessAuthDeptModel struct {
	Id         uint64       `db:"id"`
...
func GetDeptList(ctx context.Context, svcCtx *svc.ServiceContext, req *system.GetDeptListRequest) ([]*BusinessAuthDeptModel, error) {
	whereMap := gmap.New()
	if req.BusinessId != 0 {
	...
	var list []*BusinessAuthDeptModel
	resp := svcCtx.DB.Model("business_auth_dept").Where(whereMap).OrderBy("weigh").Select(ctx, &list)
	if resp.GetError() != nil {
		return nil, resp.GetError()
	}
	return list, nil
}
func GetDeptParent(ctx context.Context, svcCtx *svc.ServiceContext, req *system.GetDeptParentRequest) ([]*BusinessAuthDeptModel, error) {
	whereMap := gmap.New()
	if req.BusinessId != 0 {
		whereMap.Set("business_id =?", req.BusinessId)
	}
	...
	var list []*BusinessAuthDeptModel
	resp := svcCtx.DB.Model("business_auth_dept").Fields("id,pid,name").Where(whereMap).OrderBy("weigh").Select(ctx, &list)
	if resp.GetError() != nil {
		return nil, resp.GetError()
	}
	return list, nil
...
func SaveDept(ctx context.Context, svcCtx *svc.ServiceContext, req *system.SaveDeptRequest) (uint64, error) {
	resp := svcCtx.DB.Model("business_auth_dept").Data(req).Save(ctx)
	if resp.GetError() != nil {
...
func UpStatusDept(ctx context.Context, svcCtx *svc.ServiceContext, req *system.UpStatusDeptRequest) (uint64, error) {
	resp := svcCtx.DB.Model("business_auth_dept").Where("id =?", req.Id).Data(map[string]interface{}{"status": req.Status}).Update(ctx)
	if resp.GetError() != nil {
		return 0, resp.GetError()
	}
	return ga.Uint64(resp.GetLastId()), nil
...
func DelDept(ctx context.Context, svcCtx *svc.ServiceContext, req *system.DelDeptRequest) error {
	resp := svcCtx.DB.Model("business_auth_dept").Where("id =?", req.Id).Delete(ctx)
	if resp.GetError() != nil {
