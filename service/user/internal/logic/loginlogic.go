...
func (l *LoginLogic) Login(in *user.LoginRequest) (*user.LoginResponse, error) {
	...
	if in.Username != "" {
		...
		if pass != account.Password {
			// 添加日志操作
			go model.AddloginLog(context.Background(), l.svcCtx, ga.Map{"uid": account.Id, "account_id": account.AccountId, "business_id": account.BusinessId, "type": "business", "status": 0, "des": "账号登录", "error_msg": "输入的密码不正确！", "ip": in.ClientIp, "user_agent": in.UserAgent})
		...
		//	go model.AddloginLog(context.Background(), l.svcCtx, ga.Map{"uid": account.Id, "account_id": account.AccountId, "business_id": account.BusinessId, "type": "business", "status": 0, "des": "账号登录", "error_msg": "输入的验证码不正确！"})
		...
		go model.AddloginLog(context.Background(), l.svcCtx, ga.Map{"uid": account.Id, "account_id": account.AccountId, "business_id": account.BusinessId, "type": "business", "status": 1, "des": "账号登录", "ip": in.ClientIp, "user_agent": in.UserAgent})
