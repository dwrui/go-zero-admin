...
func UpdateConfigStatusHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var req types.UpdateConfigStatusReq
		//解析参数
		...
		//验证参数
		if msg := validate.UpdateConfigStatusValidate(req); msg != "" {
			httpx.WriteJsonCtx(r.Context(), w, http.StatusOK, ga.Failed().SetMsg(msg))
			return
		}
		l := config.NewUpdateConfigStatusLogic(r.Context(), svcCtx)
		err := l.UpdateConfigStatus(&req)
		if err != nil {
