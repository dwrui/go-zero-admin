...
func GetConfigByCategoryHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var req types.GetConfigByCategoryReq
		//解析参数
		...
		//验证参数
		if msg := validate.GetConfigByCategoryValidate(req); msg != "" {
			httpx.WriteJsonCtx(r.Context(), w, http.StatusOK, ga.Failed().SetMsg(msg))
			return
		}
		l := config.NewGetConfigByCategoryLogic(r.Context(), svcCtx)
		resp, err := l.GetConfigByCategory(&req)
		if err != nil {
