...
func GetConfigDetailHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var req types.GetConfigDetailReq
		//解析参数
		...
		//验证参数
		if msg := validate.GetConfigDetailValidate(req); msg != "" {
			httpx.WriteJsonCtx(r.Context(), w, http.StatusOK, ga.Failed().SetMsg(msg))
			return
		}
		l := config.NewGetConfigDetailLogic(r.Context(), svcCtx)
		resp, err := l.GetConfigDetail(&req)
		if err != nil {
