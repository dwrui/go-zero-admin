...
func SaveConfigValueHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var req types.SaveConfigValueReq
		//解析参数
		...
		//验证参数
		if msg := validate.SaveConfigValueValidate(req); msg != "" {
			httpx.WriteJsonCtx(r.Context(), w, http.StatusOK, ga.Failed().SetMsg(msg))
			return
		}
		l := config.NewSaveConfigValueLogic(r.Context(), svcCtx)
		err := l.SaveConfigValue(&req)
		if err != nil {
