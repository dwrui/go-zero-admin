...
func GetRoleParentHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var req types.GetRoleParentReq
		//解析参数
		if err := ga.ResData(r, &req); err != nil {
			httpx.WriteJsonCtx(r.Context(), w, http.StatusOK, ga.Failed().SetMsg(err.Error()))
			return
		}
		if msg := validate.GetParentValidate(req); msg != "" {
			httpx.WriteJsonCtx(r.Context(), w, http.StatusOK, ga.Failed().SetMsg(msg))
			return
		}
		l := role.NewGetRoleParentLogic(r.Context(), svcCtx)
		resp, err := l.GetRoleParent(&req)
		if err != nil {
			...
			httpx.WriteJsonCtx(r.Context(), w, http.StatusOK, ga.Success().SetData(resp.List))
		}
	}
}
